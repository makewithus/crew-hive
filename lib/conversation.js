...
const crewThankyouMsg = () => {
  ...
  return buildText(
    `🙏 *Thank you for registering with CrewHive!*\n\n` +
      `Your crew profile has been submitted and is *pending approval* by our team.\n\n` +
      `We will notify you here on WhatsApp once your profile is approved.\n\n` +
      `In the meantime, complete your full profile at:\n🔗 ${appUrl}\n\n` +
      `_You can log in and fill remaining details, but you will go live only after approval._`,
    STEPS.CREW_COMPLETE,
  );
};
...
export const handleMessage = async ({ userId, message }) => {
  ...
  try {
    ...
    // ── Atomic dedup claim — guarantees zero duplicates across concurrent webhooks ──
    // MSG91 fires TWO webhooks per list/button tap (interactive + text fallback).
    // Both arrive within ~1s. We use a Firestore transaction to atomically claim
    // the (norm, timestamp) slot. The second arrival finds it already claimed → dropped.
    ...
    await adminDb().runTransaction(async (tx) => {
      ...
      if (lastN === norm && Date.now() - lastT < DEDUP_WINDOW_MS) {
        // Already claimed — this is a duplicate
        dedupClaimed = false;
        return;
      }

      // Claim this norm slot atomically
      const claimPayload = { lastMsgNorm: norm, lastMsgAt: Date.now() };
      if (snap.exists) {
        tx.update(ref, claimPayload);
      } else {
        tx.set(ref, {
    ...
    if (!dedupClaimed) {
      logger.warn("[ConversationEngine] Dedup HIT (atomic) | norm:", norm, "| user:", phoneToDocId(userId));
