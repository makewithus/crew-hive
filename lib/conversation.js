...
const finalizeOrganizerProfile = async (userId, phone) => {
  ...
  await adminDb().collection("organizers").doc(id).set({
    ...
    role: 'organizer', status: 'pending', approved: false,
    createdAt: ts, updatedAt: ts,
  }, { merge: true });

  await saveUserState(userId, {
    step: STEPS.EMP_COMPLETE, role: 'organizer', phone, approved: false,
...
const resumeStep = (step, userState, userId) => {
  switch (step) {
    ...
    case STEPS.EMP_COMPLETE:
      if (userState?.approved) return txt(`✅ Your organizer profile is approved!\n\n🔗 Dashboard: ${APP_URL()}`, step);
      return txt("⏳ Your organizer profile is still *under review*. We'll notify you once approved.", step);
    default:
