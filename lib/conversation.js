...
export const handleMessage = async ({ userId, message }) => {
  ...
  try {
    ...
    // Resume
    if (/^(hi+|hey|hello|hii+|start|helo)$/i.test(msg)) {
      // If already complete, send app link
      if (step === STEPS.CREW_COMPLETE || step === STEPS.EMP_COMPLETE) {
        ...
        return buildText(
          `👋 Welcome back to *CrewHive!*\n\nYour profile is already set up. Log in here:\n🔗 ${appUrl}\n\nType *restart* to start over.`,
          step,
        );
      }
    ...
    if (step === STEPS.EMP_REQUIREMENTS) {
      ...
      const empTs = new Date().toISOString();
...
// Note: every code path above returns via buildText/buildButtons/buildList which always include a `text` field.
...
// sendBookingNotificationToCrew has moved to lib/booking-notify.js (client-safe)
// Import it from there for client components (organizer pages).
