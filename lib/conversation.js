...
const APP_URL = () =>
  process.env.NEXT_PUBLIC_APP_URL || "https://crew-hive-one.vercel.app";
...
const btns = (text, buttons, step, header = null) => ({
  type: "buttons",
  text,
  buttons,
  step,
...
const list = (text, sections, step, buttonText = "Select", header = null) => ({
  type: "list",
  text,
  sections,
  step,
...
Your organizer profile is created and submitted for admin review.\n\n` +
      `Once approved, you'll receive a WhatsApp notification with your dashboard link.\n\n` +
      `_Sit tight — we'll notify you here!_ 🙏`,
    STEPS.EMP_COMPLETE,
  );
...
const isRoleButtonTap = (norm) =>
  /^(crew|organizer|employer|yes|no|1|2)$/.test(norm);
...
export const handleMessage = async ({ userId, message }) => {
  ...
  try {
    ...
    // ── Content-based dedup (60s window) + getUserState — run in PARALLEL ──
    // This saves one full Firestore round-trip compared to sequential awaits.
    ...
    const [dedupResult, userStateResult] = await Promise.allSettled([
      adminDb().collection("dedup").doc(dedupId).create({ ts: Date.now(), phone: userId }),
      getUserState(userId),
    ]);

    // Check dedup — if the doc already existed, this is a duplicate
    if (dedupResult.status === "rejected") {
      const err = dedupResult.reason;
    ...
    const resolvedUserState = userStateResult.status === "fulfilled" ? userStateResult.value : null;
    ...
    const userState = resolvedUserState;
    const step = userState.step;

    // ── Restart — fully wipe all Firestore docs so the user starts completely fresh ──
    if (norm === "restart" || norm === "reset") {
      const digits = phoneToDocId(userId);
      const db = adminDb();

      // Delete user doc + role docs — in parallel
      await Promise.allSettled(
        ["users", "crew", "organizers", "employers"].map(async (col) => {
      ...
      // Delete all dedup entries for this user
      try {
        const snap = await db.collection("dedup").get();
      ...
      // Create a fresh user doc at role_selection step
      await saveUserState(userId, {
        step: STEPS.ROLE_SELECTION,
        role: null,
        data: {},
    ...
    if (/^(hi+|hey|hello+|hii+|start|helo|yo)$/i.test(norm)) {
      ...
      if (step === STEPS.EMP_COMPLETE) {
        if (userState.approved) {
          return txt(
            `👋 Welcome back!\n\nYour organizer profile is approved ✅\n\n🔗 Dashboard: ${APP_URL()}\n\nType *restart* to start fresh.`,
            step,
          );
        }
        return txt(
          "👋 Welcome back!\n\nYour organizer profile is *pending admin approval*. We'll notify you here once approved.\n\nType *restart* to start over.",
          step,
        );
      }
    ...
    if (step === STEPS.ROLE_SELECTION) {
      ...
      if (isOrganizer) {
        const moved = await atomicStepTransition(
          userId,
          STEPS.ROLE_SELECTION,
          { role: "organizer", step: STEPS.EMP_NAME },
          norm,
        );
        if (!moved) return DUPLICATE_RESPONSE;
        return txt(
    ...
    if (step === STEPS.CREW_CITY_OTHER) {
      ...
      if (!city || city.length < 2)
        return txt("Please enter a valid city name:", STEPS.CREW_CITY_OTHER);
      const moved = await atomicStepTransition(
        userId,
    ...
    if (step === STEPS.CREW_EXPERIENCE) {
      ...
      if (!exp) return DUPLICATE_RESPONSE;
      const moved = await atomicStepTransition(
        userId,
    ...
    if (step === STEPS.CREW_PHONE_CONFIRM) {
      const isYes =
        norm === "yes" || norm.includes("yes") || norm.includes("use this");
      const isNo =
        norm === "no" || norm.includes("no") || norm.includes("different");
      if (isYes) {
        const moved = await atomicStepTransition(
          userId,
    ...
    if (step === STEPS.EMP_COMPANY) {
      ...
      if (!company || company.length < 1)
        return txt("Please enter your company name:", STEPS.EMP_COMPANY);
      const moved = await atomicStepTransition(
        userId,
        STEPS.EMP_COMPANY,
    ...
    if (step === STEPS.EMP_CITY_OTHER) {
      const city = message.trim();
      if (!city || city.length < 2)
        return txt("Please enter a valid city name:", STEPS.EMP_CITY_OTHER);
      const moved = await atomicStepTransition(
        userId,
    ...
    if (step === STEPS.EMP_PHONE_CONFIRM) {
      ...
      if (isYes) {
        ...
        if (!moved) return DUPLICATE_RESPONSE;
        return await finalizeOrganizerProfile(
          userId,
          `+${phoneToDocId(userId)}`,
        );
      }
    ...
    // ── Already completed — only respond to explicit keywords, drop everything else
    if (step === STEPS.CREW_COMPLETE || step === STEPS.EMP_COMPLETE) {
      // Greetings already handled above; restart already handled above.
      // Everything else (stale button taps, MSG91 fallback texts) → drop silently.
      return DUPLICATE_RESPONSE;
    }
    ...
    if (step === STEPS.BOOKING_RESPONSE) {
      ...
      if (bookingId) {
        const bookingSnap = await adminDb()
          ...
          .doc(bookingId)
          .get();
        const bookingStatus = bookingSnap.exists
          ? bookingSnap.data()?.status
          : null;

        if (!bookingSnap.exists || bookingStatus !== "pending") {
          await saveUserState(userId, {
            step: STEPS.CREW_COMPLETE,
            pendingBookingId: null,
          });
...
const resumeStep = (step, userState, userId) => {
  switch (step) {
    ...
    case STEPS.CREW_NAME:
      return txt("What is your *full name*?", step);
    case STEPS.EMP_NAME:
      return txt("What is your *full name*?", step);
    ...
    case STEPS.EMP_CITY_OTHER:
      return txt("Please *type your city name*:", step);
    ...
    case STEPS.EMP_COMPANY:
      return txt("What is your *company / production house name*?", step);
    case STEPS.EMP_REQUIREMENTS:
      return txt("Please describe your *hiring requirements*:", step);
    ...
    case STEPS.EMP_COMPLETE:
      ...
      return txt(
        "⏳ Your organizer profile is still *under review*. We'll notify you once approved.",
        step,
      );
    default:
