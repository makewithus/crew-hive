...
const saveUserState = async (phone, updates) => {
  try {
    ...
    console.log('[ConversationEngine] saveUserState ok | phone:', phoneToDocId(phone), '| keys:', Object.keys(updates).join(', '));
  } catch (err) {
    console.error('[ConversationEngine] saveUserState error:', err.message, '| phone:', phoneToDocId(phone));
    throw err;
  }
};
...
export const handleMessage = async ({ userId, message }) => {
  console.log('[ConversationEngine] Incoming | userId:', userId, '| message:', JSON.stringify(message));
  try {
