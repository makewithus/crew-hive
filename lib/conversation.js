 ...
 * Prevents double-processing when MSG91 sends the same button tap as both
 * interactive AND text payloads (different fingerprints, both pass dedup).
 */
const atomicStepTransition = async (phone, expectedStep, updates) => {
  const ref = adminDb().collection('users').doc(phoneToDocId(phone));
  try {
    let moved = false;
    await adminDb().runTransaction(async (tx) => {
      const snap = await tx.get(ref);
      const current = snap.exists ? snap.data()?.step : null;
      ...
      if (snap.exists) {
        ...
        tx.set(ref, { phone: `+${phoneToDocId(phone)}`, whatsappPhone: `+${phoneToDocId(phone)}`, createdAt: new Date().toISOString(), ...payload });
      }
      moved = true;
    });
    return moved;
  } catch (err) {
    logger.error('[ConversationEngine] atomicStepTransition error:', err.message);
...
const normaliseMsg = (raw) => {
  ...
  // Strip leading number + dot + space:  "10. Other" → "other"
  s = s.replace(/^\d+\.\s*/, '');
...
export const handleMessage = async ({ userId, message }) => {
  ...
  try {
    // msg = raw lowercase; norm = stripped of "N. " prefix and emoji prefix
    const msg  = message.trim().toLowerCase();
    const norm = normaliseMsg(message);

    // ── Per-user atomic dedup ──────────────────────────────────────────────────
    // MSG91 retries the SAME inbound message 2-3x (within seconds).
    // We store lastMsgNorm + lastMsgAt on the user doc and check atomically here.
    // This also handles the race condition where MSG91 retries arrive before the
    // previous request's after() background write commits.
    const userRef = adminDb().collection('users').doc(phoneToDocId(userId));
    const DEDUP_WINDOW_MS = 30_000; // 30 s is far more than enough for retries
    let userState = null;
    ...
    await adminDb().runTransaction(async (tx) => {
      ...
      userState = data;
      const now   = Date.now();
      const lastN = data?.lastMsgNorm;
      const lastT = data?.lastMsgAt  || 0;
      if (lastN === norm && (now - lastT) < DEDUP_WINDOW_MS) {
        logger.warn('[ConversationEngine] Per-user dedup HIT | norm:', norm, '| age(ms):', now - lastT);
        isDuplicate = true;
        return; // don't write
      }
      // Mark message as being-processed so any concurrent retry sees it
      const markPayload = { lastMsgNorm: norm, lastMsgAt: now };
      if (snap.exists) {
        tx.update(userRef, markPayload);
      } else {
        tx.set(userRef, {
          phone: `+${phoneToDocId(userId)}`,
          whatsappPhone: `+${phoneToDocId(userId)}`,
          createdAt: new Date().toISOString(),
        ...
        // userState is null → brand-new user, handled below
      }
    });

    if (isDuplicate) return DUPLICATE_RESPONSE;

    // ── Brand-new user ─────────────────────────────────────────────────────────
    if (!userState) {
      // Doc was already created in the dedup transaction above (step: ROLE_SELECTION)
    ...
    if (step === STEPS.ROLE_SELECTION) {
      ...
      const isEmployer = norm === 'employer' || norm === '2' || norm.includes('employer');

      if (isCrew) {
        const moved = await atomicStepTransition(userId, STEPS.ROLE_SELECTION, { role: 'crew', step: STEPS.CREW_NAME });
        if (!moved) return DUPLICATE_RESPONSE;
        return buildText('\ud83d\udc77 *Crew Member* selected!\n\nWhat is your *full name*?', STEPS.CREW_NAME);
      }
      if (isEmployer) {
        const moved = await atomicStepTransition(userId, STEPS.ROLE_SELECTION, { role: 'employer', step: STEPS.EMP_NAME });
        if (!moved) return DUPLICATE_RESPONSE;
    ...
    if (step === STEPS.CREW_NAME) {
      ...
      const moved = await atomicStepTransition(userId, STEPS.CREW_NAME, { 'data.name': name, step: STEPS.CREW_CITY });
      if (!moved) return DUPLICATE_RESPONSE;
    ...
    if (step === STEPS.CREW_CITY) {
      if (norm === 'other') {
        const moved = await atomicStepTransition(userId, STEPS.CREW_CITY, { step: STEPS.CREW_CITY_OTHER });
        if (!moved) return DUPLICATE_RESPONSE;
        return buildText('Please *type your city name*:', STEPS.CREW_CITY_OTHER);
      }
      const city = CITIES.find((c) => c.toLowerCase().replace(/\s/g, '_') === norm.replace(/\s/g, '_'));
      if (!city) {
        logger.warn('[ConversationEngine] CREW_CITY: no city match for norm:', norm, '— likely a stale retry, dropping silently');
        return DUPLICATE_RESPONSE;
      }
      const moved = await atomicStepTransition(userId, STEPS.CREW_CITY, { 'data.city': city, step: STEPS.CREW_ROLE });
      if (!moved) return DUPLICATE_RESPONSE;
    ...
    if (step === STEPS.CREW_CITY_OTHER) {
      ...
      const moved = await atomicStepTransition(userId, STEPS.CREW_CITY_OTHER, { 'data.city': city, step: STEPS.CREW_ROLE });
      if (!moved) return DUPLICATE_RESPONSE;
      return crewRoleListMsg(STEPS.CREW_ROLE);
    }

    if (step === STEPS.CREW_ROLE) {
      const idxStr = msg.match(/^(\d+)/)?.[1];
      const idx = idxStr ? parseInt(idxStr, 10) : NaN;
      const byName = isNaN(idx) ? CREW_ROLES.findIndex((r) => norm.includes(r.toLowerCase())) : -1;
      const roleIdx = (!isNaN(idx) && idx >= 1 && idx <= CREW_ROLES.length) ? idx - 1 : byName;
      if (roleIdx < 0) {
        logger.warn('[ConversationEngine] CREW_ROLE: no role match for norm:', norm, '— likely a stale retry, dropping silently');
        return DUPLICATE_RESPONSE;
      }
      const role = CREW_ROLES[roleIdx];
      const moved = await atomicStepTransition(userId, STEPS.CREW_ROLE, { 'data.crewRole': role, step: STEPS.CREW_EXPERIENCE });
      if (!moved) return DUPLICATE_RESPONSE;
      return experienceListMsg(STEPS.CREW_EXPERIENCE);
    }

    if (step === STEPS.CREW_EXPERIENCE) {
      const expNum = msg.match(/^(\d+)/)?.[1];
      ...
      const moved = await atomicStepTransition(userId, STEPS.CREW_EXPERIENCE, { 'data.experience': exp, step: STEPS.CREW_PHONE_CONFIRM });
      if (!moved) return DUPLICATE_RESPONSE;
      return phoneConfirmMsg(userId, STEPS.CREW_PHONE_CONFIRM);
    }

    if (step === STEPS.CREW_PHONE_CONFIRM) {
      const isYes = norm === 'yes' || norm === 'y' || norm.includes('yes') || norm.includes('use this');
      const isNo  = norm === 'no'  || norm === 'n' || norm.includes('no')  || norm.includes('different');
      if (isYes) {
        const moved = await atomicStepTransition(userId, STEPS.CREW_PHONE_CONFIRM, { step: STEPS.CREW_COMPLETE });
        if (!moved) return DUPLICATE_RESPONSE;
        await saveUserState(userId, { step: STEPS.CREW_PHONE_CONFIRM });
        return await finalizeCrewProfile(userId, `+${phoneToDocId(userId)}`);
      }
      if (isNo) {
        const moved = await atomicStepTransition(userId, STEPS.CREW_PHONE_CONFIRM, { step: STEPS.CREW_PHONE_MANUAL });
        if (!moved) return DUPLICATE_RESPONSE;
    ...
    if (step === STEPS.EMP_NAME) {
      ...
      const moved = await atomicStepTransition(userId, STEPS.EMP_NAME, { 'data.name': name, step: STEPS.EMP_COMPANY });
      if (!moved) return DUPLICATE_RESPONSE;
    ...
    if (step === STEPS.EMP_COMPANY) {
      ...
      const moved = await atomicStepTransition(userId, STEPS.EMP_COMPANY, { 'data.company': company, step: STEPS.EMP_CITY });
      if (!moved) return DUPLICATE_RESPONSE;
    ...
    if (step === STEPS.EMP_CITY) {
      if (norm === 'other') {
        const moved = await atomicStepTransition(userId, STEPS.EMP_CITY, { step: STEPS.EMP_CITY_OTHER });
        if (!moved) return DUPLICATE_RESPONSE;
        return buildText('Please *type your city name*:', STEPS.EMP_CITY_OTHER);
      }
      const city = CITIES.find((c) => c.toLowerCase().replace(/\s/g, '_') === norm.replace(/\s/g, '_'));
      if (!city) {
        logger.warn('[ConversationEngine] EMP_CITY: no city match for norm:', norm, '— likely a stale retry, dropping silently');
        return DUPLICATE_RESPONSE;
      }
      const moved = await atomicStepTransition(userId, STEPS.EMP_CITY, { 'data.city': city, step: STEPS.EMP_REQUIREMENTS });
      if (!moved) return DUPLICATE_RESPONSE;
    ...
    if (step === STEPS.EMP_CITY_OTHER) {
      ...
      const moved = await atomicStepTransition(userId, STEPS.EMP_CITY_OTHER, { 'data.city': city, step: STEPS.EMP_REQUIREMENTS });
      if (!moved) return DUPLICATE_RESPONSE;
    ...
    if (step === STEPS.EMP_REQUIREMENTS) {
      ...
      const moved = await atomicStepTransition(userId, STEPS.EMP_REQUIREMENTS, { 'data.requirements': req, step: STEPS.EMP_PHONE_CONFIRM });
      if (!moved) return DUPLICATE_RESPONSE;
    ...
    if (step === STEPS.EMP_PHONE_CONFIRM) {
      ...
      const isNo  = norm === 'no'  || norm === 'n' || norm.includes('no')  || norm.includes('different');
      if (isYes) {
        const moved = await atomicStepTransition(userId, STEPS.EMP_PHONE_CONFIRM, { step: STEPS.EMP_COMPLETE });
        if (!moved) return DUPLICATE_RESPONSE;
        await saveUserState(userId, { step: STEPS.EMP_PHONE_CONFIRM });
        return await finalizeEmployerProfile(userId, `+${phoneToDocId(userId)}`);
      }
      if (isNo) {
        const moved = await atomicStepTransition(userId, STEPS.EMP_PHONE_CONFIRM, { step: STEPS.EMP_PHONE_MANUAL });
        if (!moved) return DUPLICATE_RESPONSE;
