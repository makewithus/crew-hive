  ...
  try {
    ...
    await adminDb().runTransaction(async (tx) => {
      ...
      if (current !== expectedStep) {
        // Step already advanced by a concurrent request — skip
        moved = false;
        return;
      }
      const payload = { ...updates, updatedAt: new Date().toISOString() };
  ...
  try {
    ...
    if (step === STEPS.ROLE_SELECTION) {
      ...
      if (isCrew) {
        ...
        if (!moved) {
          // Already processed by a concurrent duplicate — re-read and resume
          const fresh = await getUserState(userId);
          return await resumeStep(fresh?.step || STEPS.CREW_NAME, fresh, userId);
        }
      ...
      if (isEmployer) {
        ...
        if (!moved) {
          const fresh = await getUserState(userId);
          return await resumeStep(fresh?.step || STEPS.EMP_NAME, fresh, userId);
        }
    ...
    if (step === STEPS.CREW_NAME) {
      ...
      if (!moved) return cityListMsg(STEPS.CREW_CITY);
      return cityListMsg(STEPS.CREW_CITY);
    }

    if (step === STEPS.CREW_CITY) {
      // norm strips "N. " prefix: "10. Other" → "other", "1. Kochi" → "kochi"
      if (norm === 'other') {
        const moved = await atomicStepTransition(userId, STEPS.CREW_CITY, { step: STEPS.CREW_CITY_OTHER });
        if (!moved) return buildText('Please *type your city name*:', STEPS.CREW_CITY_OTHER);
      ...
      if (!moved) return crewRoleListMsg(STEPS.CREW_ROLE);
    ...
    if (step === STEPS.CREW_ROLE) {
      // Interactive sends id="1".."10"; text sends "1. Cinematographer" — extract leading digit
      const idxStr = msg.match(/^(\d+)/)?.[1];
      const idx = idxStr ? parseInt(idxStr, 10) : NaN;
      // Also try matching by role name in norm
      ...
      if (roleIdx < 0) return crewRoleListMsg(STEPS.CREW_ROLE);
      ...
      if (!moved) return experienceListMsg(STEPS.CREW_EXPERIENCE);
      return experienceListMsg(STEPS.CREW_EXPERIENCE);
    }

    if (step === STEPS.CREW_EXPERIENCE) {
      // Interactive sends id="1".."4"; text sends "1. 0–2 years (...)" — extract leading digit
      ...
      if (!moved) return phoneConfirmMsg(userId, STEPS.CREW_PHONE_CONFIRM);
    ...
    if (step === STEPS.CREW_PHONE_CONFIRM) {
      ...
      if (isYes) {
        ...
        if (!moved) return thankyouMsg('crew'); // Already finalized
        // Restore step so finalizeCrewProfile reads correct state
      ...
      if (isNo) {
        ...
        if (!moved) return buildText('Please type your *phone number* for OTP login:\n\n_Example: +91XXXXXXXXXX_', STEPS.CREW_PHONE_MANUAL);
    ...
    if (step === STEPS.EMP_COMPANY) {
      ...
      if (!moved) return cityListMsg(STEPS.EMP_CITY);
    ...
    if (step === STEPS.EMP_CITY) {
      if (norm === 'other') {
        ...
        if (!moved) return buildText('Please *type your city name*:', STEPS.EMP_CITY_OTHER);
    ...
    if (step === STEPS.EMP_REQUIREMENTS) {
      ...
      if (!moved) return phoneConfirmMsg(userId, STEPS.EMP_PHONE_CONFIRM);
    ...
    if (step === STEPS.EMP_PHONE_CONFIRM) {
      ...
      if (isYes) {
        ...
        if (!moved) return thankyouMsg('employer');
