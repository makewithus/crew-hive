...
export const handleMessage = async ({ userId, message }) => {
  ...
  try {
    ...
    if (step === STEPS.ROLE_SELECTION) {
      ...
      const isCrew     = msg === 'crew' || msg === '1' || msg.includes('crew');
      const isEmployer = msg === 'employer' || msg === '2' || msg.includes('employer');
    ...
    if (step === STEPS.CREW_PHONE_CONFIRM) {
      const isYes = msg === 'yes' || msg === 'y' || msg.includes('yes') || msg.includes('use this');
      const isNo  = msg === 'no'  || msg === 'n' || msg.includes('no')  || msg.includes('different');
      if (isYes) {
