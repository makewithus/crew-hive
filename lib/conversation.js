...
const normaliseMsg = (raw) => {
  ...
  if (s.startsWith('{') || s.startsWith('[')) {
    ...
    } catch (_) { /* not JSON — leave s as-is */ }
  }

  s = s.toLowerCase();
  // Strip leading number + dot + space:  "10. Other" → "other"
  // BUT preserve bare numbers like "3" (used as expMap/roleMap keys)
  s = s.replace(/^\d+\.\s+/, '');
...
export const handleMessage = async ({ userId, message }) => {
  ...
  try {
    ...
    // ── Fast per-user dedup (single read, no transaction overhead) ────────────
    // Read user state once. If lastMsgNorm matches within 30s → already processing.
    // Correctness is guaranteed by atomicStepTransition; this is just a fast early-exit.
    const DEDUP_WINDOW_MS = 8_000; // MSG91 retries arrive within 2-3s; 8s is more than enough
    const userSnap = await adminDb().collection('users').doc(phoneToDocId(userId)).get();
    const userState = userSnap.exists ? userSnap.data() : null;

    if (userState) {
      const lastN = userState.lastMsgNorm;
      const lastT = userState.lastMsgAt || 0;
      if (lastN === norm && (Date.now() - lastT) < DEDUP_WINDOW_MS) {
        logger.warn('[ConversationEngine] Dedup HIT | norm:', norm, '| age(ms):', Date.now() - lastT);
    ...
    if (!userState) {
      await saveUserState(userId, {
        step: STEPS.ROLE_SELECTION, role: null, data: {},
        lastMsgNorm: norm, lastMsgAt: Date.now(),
      });
      return [
        buildText(
    ...
    if (step === STEPS.ROLE_SELECTION) {
      ...
      if (isEmployer) {
        const moved = await atomicStepTransition(userId, STEPS.ROLE_SELECTION, { role: 'employer', step: STEPS.EMP_NAME }, norm);
        if (!moved) return DUPLICATE_RESPONSE;
    ...
    if (step === STEPS.CREW_CITY) {
      ...
      const cityById   = CITIES.find((c) => c.toLowerCase().replace(/\s/g, '_') === norm.replace(/\s/g, '_'));
      const cityByName = CITIES.find((c) => c.toLowerCase() === norm.toLowerCase());
      const idxNum     = parseInt(norm, 10);
      const cityByIdx  = (!isNaN(idxNum) && idxNum >= 1 && idxNum <= CITIES.length) ? CITIES[idxNum - 1] : null;
      const city       = cityById || cityByName || cityByIdx;
      logger.log('[ConversationEngine] CREW_CITY | norm:', norm, '| matched:', city || 'NONE');
      if (!city) {
        logger.warn('[ConversationEngine] CREW_CITY: no city match — dropping silently');
        return DUPLICATE_RESPONSE;
      }
    ...
    if (step === STEPS.EMP_CITY) {
      ...
      const cityByIdx2 = (!isNaN(idxNum2) && idxNum2 >= 1 && idxNum2 <= CITIES.length) ? CITIES[idxNum2 - 1] : null;
      const city       = cityById || cityByName || cityByIdx2;
      logger.log('[ConversationEngine] EMP_CITY | norm:', norm, '| matched:', city || 'NONE');
      if (!city) {
        logger.warn('[ConversationEngine] EMP_CITY: no city match — dropping silently');
        return DUPLICATE_RESPONSE;
      }
