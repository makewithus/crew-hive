...
export const updateCrewProfile = async (phone, crewData) => {
  try {
    // Use server API (Admin SDK) to bypass Firestore client rules
    let idToken = null;
    try { idToken = auth.currentUser ? await auth.currentUser.getIdToken() : null; } catch (_) {}
    const res = await fetch("/api/crew/update-profile", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        ...(idToken ? { Authorization: `Bearer ${idToken}` } : {}),
      },
      body: JSON.stringify({ phone, data: crewData }),
    });
