...
export const approveCrewMember = async (phone) => {
  try {
    ...
    await Promise.all([
      updateDoc(doc(db, 'crew', id), { status: 'approved', approvedAt: ts, updatedAt: ts }),
      updateDoc(doc(db, 'users', id), { approved: true, approvedAt: ts, updatedAt: ts }),
    ]);
    return { success: true };
  } catch (error) {
...
export const rejectCrewMember = async (phone) => {
  try {
    const id = phoneToDocId(phone);
    const ts = new Date().toISOString();
    await updateDoc(doc(db, 'crew', id), { status: 'rejected', rejectedAt: ts, updatedAt: ts });
    return { success: true };
  } catch (error) {
...
export const getPendingCrew = async () => {
  try {
    const snap = await getDocs(query(collection(db, 'crew'), where('status', '==', 'pending')));
...
export const getApprovedCrew = async (filters = {}) => {
  try {
    const snap = await getDocs(query(collection(db, 'crew'), where('status', '==', 'approved')));
...
export const getEmployerProfile = async (phone) => {
  try {
    const snap = await getDoc(doc(db, 'employers', phoneToDocId(phone)));
    if (snap.exists()) return { success: true, data: { id: snap.id, ...snap.data() } };
...
export const getCrewBookings = async (phone) => {
  try {
    const snap = await getDocs(query(collection(db, 'bookings'), where('crewId', '==', phoneToDocId(phone))));
    return { success: true, data: snap.docs.map((d) => ({ id: d.id, ...d.data() })) };
  } catch (error) {
...
export const getEmployerBookings = async (phone) => {
  try {
    const snap = await getDocs(query(collection(db, 'bookings'), where('employerId', '==', phoneToDocId(phone))));
    return { success: true, data: snap.docs.map((d) => ({ id: d.id, ...d.data() })) };
  } catch (error) {
...
export const getSuperAdminStats = async () => {
  try {
    const [usersSnap, crewSnap, employersSnap, bookingsSnap] = await Promise.all([
