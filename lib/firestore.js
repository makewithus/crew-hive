...
export const getOrganizerBookings = async (phone) => {
  try {
    const id = phoneToDocId(phone);
    const [organizerRes, employerRes] = await Promise.all([
      dbCall({ op: "query", collection: "bookings", filters: [{ field: "organizerId", op: "==", value: id }] }),
      dbCall({ op: "query", collection: "bookings", filters: [{ field: "employerId", op: "==", value: id }] }),
    ]);
    const byId = new Map();
    for (const booking of [...organizerRes.data, ...employerRes.data]) {
      byId.set(booking.id, booking);
    }
    return { success: true, data: Array.from(byId.values()) };
  } catch (error) {
    console.error("[Firestore] getOrganizerBookings error:", error);
    return { success: false, error: error.message };
  }
};
