...
const FIREBASE_ERRORS = {
  ...
  'auth/operation-not-allowed':
    'Phone sign-in is not enabled. Go to Firebase Console → Authentication → ' +
  ...
  'auth/internal-error':
    'Firebase internal error. Make sure this domain is added in Firebase Console ' +
    '→ Authentication → Settings → Authorized domains.',
  'auth/missing-app-credential':
    'reCAPTCHA setup error. Please refresh the page and try again.',
  'auth/missing-client-identifier':
    'reCAPTCHA setup error. Please refresh the page and try again.',
  'auth/web-storage-unsupported':
    'Browser storage is blocked. Please enable cookies/storage and try again.',
  'auth/user-disabled':
...
const friendlyError = (code) => {
  const mapped = FIREBASE_ERRORS[code];
  if (mapped) return mapped;
  // Show the raw code so it can be reported/debugged
  return `Something went wrong (${code || 'unknown'}). Please refresh and try again.`;
};
