...
const CITIES = ['All Cities', 'Kochi', 'Trivandrum', 'Kozhikode', 'Thrissur', 'Kannur', 'Kottayam', 'Malappuram', 'Palakkad', 'Alappuzha'];

export default function EmployerDashboard() {
  const { isAuthenticated, userRole, userApproved, userPhone, loading, logout } = useAuth();
  ...
  useEffect(() => {
    if (!loading) {
      ...
      else if (userRole === 'employer' && !userApproved) router.push('/crew/verify');
    }
  ...
  return (
    <div className="min-h-screen bg-[#0D0D0D] text-white">
      ...
      {bookingModal && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-40 px-4">
          <div className="bg-[#1A1A1A] border border-zinc-700 rounded-2xl p-6 w-full max-w-md">
            <h2 className="text-white font-semibold text-lg mb-1">Book {bookingModal.name}</h2>
            <p className="text-zinc-400 text-sm mb-4">{bookingModal.role} · {bookingModal.city}</p>
            <textarea
              value={jobDetails}
            ...
            <div className="flex gap-2 mt-4">
              <button
                onClick={() => { setBookingModal(null); setJobDetails(''); }}
      ...
      <header className="border-b border-zinc-800 px-6 py-4 flex items-center justify-between">
        ...
        <button onClick={logout} className="text-zinc-400 hover:text-white text-sm transition-colors">
          Sign out
        </button>
      </header>
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        <div className="mb-6">
          ...
          <p className="text-zinc-400 text-sm mt-1">Browse approved crew members available for hire</p>
        </div>
        ...
        {/* Grid */}
        {fetching ? (
          <div className="text-zinc-400 text-sm py-12 text-center">Loading crew...</div>
        ) : crew.length === 0 ? (
